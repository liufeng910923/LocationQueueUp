...
import com.queueup.adapters.ContactAdapter;
import com.queueup.conf.DemoApplication;
import com.queueup.entity.ContactInfo;

public class ContactListView extends Activity {
	
	public static ContactListView instance;
	ListView listView;
	...
	ArrayList<ContactInfo> contactChecked = new ArrayList<ContactInfo>();
	//选中的手机号
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView parent, View view,
				...
				if (cb.isChecked()) {
					...
					contactChecked.remove(contactList.get(position));
				} else {
					cb.setChecked(true);
					numberStr += ","
							+ contactList.get(position).getUserNumber();
					contactList.get(position).isChecked = true;
					contactChecked.add(contactList.get(position));
				}
			}
		});	
        ...
        DemoApplication.getInstance().add(ContactListView.this);
	...
	private OnClickListener btnClick = new OnClickListener() {
		public void onClick(View v) {
			...
			/*		for(int i =0;i<contactChecked.size();i++){
						ContactInfo contactInfo = contactChecked.get(i);
						String name = contactInfo.getContactName();
						String phone = contactInfo.getUserNumber();
						String loginame = AndroidUtils.getSharedPreferences(
								ContactListView.instance, "LOGIN", "LOGIN_NAME");
						InsertMesgShareUtility insertMesgShareUtility = new InsertMesgShareUtility(loginame,name,phone);
...
System.out.println(strPhoneNumber);
				...
				cci.isChecked = false;
					...
					if (!IsContain(contactList, cci.userNumber)) {if (IsUserNumber(strPhoneNumber)) {
